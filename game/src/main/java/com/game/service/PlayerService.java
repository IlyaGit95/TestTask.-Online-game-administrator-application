...
import com.game.exeption.NotFoundException;
import com.game.exeption.RequestException;
...
public class PlayerService {
    ...
    private static final int MAX_EXPERIENCE_SIZE = 10000000;
    ...
    public Player createPlayer(Player player) {
        ...
        player.setLevel(currentCharacterLevel(player.getExperience()));
        player.setUntilNextLevel(experienceToTheNextLevel(player.getExperience(), player.getLevel()));
    ...
    public Integer currentCharacterLevel(Integer experience) {
        return (((int) Math.sqrt(2500 + 200 * experience)) - 50) / 100;
    }

    private Integer experienceToTheNextLevel(Integer experience, Integer level) {
    ...
    public Player updatePlayer(Long id, Player player) {
        ...
        newPlayer.setLevel(currentCharacterLevel(newPlayer.getExperience()));
        newPlayer.setUntilNextLevel(experienceToTheNextLevel(newPlayer.getExperience(), newPlayer.getLevel()));
    ...
    public void checkId(Long id) {
        if (id <= 0) throw new RequestException("ID is invalid");
    }

    public void checkName(String name) {
        if (name == null || name.isEmpty() || name.length() > MAX_NAME_LENGTH)
            throw new RequestException("Name is invalid");
    }

    public void checkProfession(Profession profession) {
        if (profession == null)
            throw new RequestException("Profession is invalid");
    }

    public void checkBirthday(Date birthday) {
        if (birthday == null)
            throw new RequestException("Birthday is invalid");
        ...
        if (calendar.get(Calendar.YEAR) < MIN_BIRTHDAY || calendar.get(Calendar.YEAR) > MAX_BIRTHDAY)
            throw new RequestException("Birthday out of bounds");
    }

    public void checkRace(Race race) {
        if (race == null)
            throw new RequestException("Race is invalid");
    }

    public void checkTitle(String title) {
        if (title.length() > MAX_TITLE_LENGTH || title == null || title.isEmpty())
            throw new RequestException("Title is invalid");
    }

    public void checkExperience(Integer experience) {
        if (experience < 0 || experience > MAX_EXPERIENCE_SIZE || experience == null)
            throw new RequestException("Experience is invalid");
    }
